...
public class UGRDF2PG extends Runnable {
    ...
    protected void run() {
        ...
        logger.debugInfo("BEGIN TO TEST RDF2PG by UG");
        try {
            Thread.sleep(10000);
        ...
        graphAPI.refreshRDF();
        logger.debugInfo("RDF2LPG end " , (System.currentTimeMillis() - begin));
        ...
        logger.debugInfo("LPG2UGM end " , (System.currentTimeMillis() - begin));
        System.out.println("ugm vertex count: " + unifiedGraph.getPairs().size());
        try {
            Thread.sleep(5000);
        } catch (InterruptedException e) {
